...
class IncidentStatus(Action):
    ...
    async def run(
        ...
        # handle anonymous profile. No calling of snow API.
        if user_profile.get("id") == anonymous_profile.get("id"):
            message = "Since you are not registered, I cant't tell your incident status :/"
        else:
            incidet_states = snow.states_db()
            incidents_result = await snow.retrieve_incidents(user_profile)
            print(incidents_result)
            incidents = incidents_result.get("incidents")
            if incidents:
                message = "\n".join(
                    [
                        f'Incident {i.get("number")}: '
                        f'"{i.get("shor_description")}", '
                        f'opened on {i.get("opened_at")} '
                        f'{incidet_states.get(i.get("incident_state"))} '
                        for i in incidents
                    ]
                )
            else:
...
class OpenIncidentForm(FormAction):
    ...
    async def submit(
        ...
        if user_profile.get("id") == anonymous_profile.get("id"):
            message = (
                "Nice try, but I can't actually create a ticket for you since you're anonymous."
            )
            dispatcher.utter_message(text=message)
            ...
            if incident_number:
                ...
                message = (
                    f"Something went wrong opening your incident. Sorry. "
                    f'{result.get("error")}'
                )
